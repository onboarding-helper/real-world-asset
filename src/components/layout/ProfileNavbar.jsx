...
const Navbar = () => {
  ...
  return (
      <nav className="h-full p-3 space-y-2 w-60 dark:bg-gray-50 dark:text-gray-800 profile-navbar">
	<div className="flex items-center p-2 space-x-4">
		<img src="https://hwchamber.co.uk/wp-content/uploads/2022/04/avatar-placeholder.gif" alt="" className="w-12 h-12 rounded-full dark:bg-gray-500" />
		<div>
			<h2 className="text-lg font-semibold"> {user?.walletAddress && `${user.walletAddress.substring(0, 6)}...${user.walletAddress.substring(user.walletAddress.length - 4)}`}</h2>
			<span className="flex items-center space-x-1">
				<a rel="noopener noreferrer" href="#" className="text-xs hover:underline dark:text-gray-600">DID</a>
			</span>
		</div>
	</div>
